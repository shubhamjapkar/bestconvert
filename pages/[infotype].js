...
function Page({ query, notFound }) {
  ...
  const [convertText, setconvertText] = useState(query.convert[0]);
  const [converttoText, setconverttoText] = useState(query.convertto[0]);
  return (
    <>
      <Head>
              ...
              {query.convert.map((e, i) => {
                ...
                return (
                  <label
                    key={i}
                    onClick={(e) => {
                      if (e.target.innerText == converttoText) {
                        alert("Same option can not be sellected")
                        return;
                      }
                      setconvertText(e.target.innerText);
                    }}
                  >
                    {e}
                  </label>
                ...
                return (
                  <label
                    ...
                    onClick={(e) => {
                      ...
                      if (e.target.innerText == convertText) {
                        alert("Same option can not be sellected")
                        return;
                      }
                      setconverttoText(e.target.innerText)
                    }}>
                    {e}
                  </label>
                );
      ...
      <div className="middle_box">
        ...
        <div className="security_box">
          <label className="security_box">
            <h2>SECURITY</h2>
            <p>
              Bestconver.in has been trusted by our users and customers since its founding in 2023. No one except you will ever have access to your files. We earn money by organic traffic, not by selling your data.
            </p>
          </label>
        </div>
      </div>
